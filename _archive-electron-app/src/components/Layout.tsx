...
export type Page = 'home' | 'deals' | 'accounts' | 'contacts' | 'leads' | 'reports' | 'settings';
...
// CSS-in-JS helper to bypass TS CSSProperties check for Electron-specific props
const dragStyle = (extra: React.CSSProperties = {}): React.CSSProperties =>
  Object.assign({ WebkitAppRegion: 'drag' } as unknown as React.CSSProperties, extra);
const noDragStyle = (extra: React.CSSProperties = {}): React.CSSProperties =>
  Object.assign({ WebkitAppRegion: 'no-drag' } as unknown as React.CSSProperties, extra);
...
export const Layout = memo(function Layout({ page, onPageChange, userName, children }: LayoutProps) {
  ...
  useEffect(() => {
    if (window.api?.getPlatform) {
      window.api.getPlatform().then((p: string) => setPlatform(p));
    }
  }, []);
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Noto Sans JP', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div style={dragStyle({
        ...
        <div style={noDragStyle({ display: 'flex', alignItems: 'center', gap: 4, marginLeft: platform === 'darwin' ? 72 : 6 })}>
          <span style={{ fontSize: 11, fontWeight: 700, color: 'transparent', width: 0 }}></span>
        </div>
        <div style={noDragStyle({ display: 'flex', alignItems: 'center', gap: 3 })}>
          {/* 検索ボタン */}
          <button style={{
            ...
            <span>検索</span>
            <span style={{ fontSize: 9, color: '#ccc', marginLeft: 4 }}>{platform === 'darwin' ? '⌘K' : 'Ctrl+K'}</span>
          </button>
          {/* ユーザーアバター */}
          {userName && (
          ...
          {isWin && (
            <div style={{ display: 'flex', gap: 1, marginLeft: 4 }}>
              <button onClick={() => window.api?.minimize?.()} style={{ width: 28, height: 22, border: 'none', background: 'transparent', cursor: 'pointer', color: '#999', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="10" height="1" viewBox="0 0 10 1"><rect width="10" height="1" fill="currentColor"/></svg>
              </button>
              <button onClick={() => window.api?.maximize?.()} style={{ width: 28, height: 22, border: 'none', background: 'transparent', cursor: 'pointer', color: '#999', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><rect x="0.5" y="0.5" width="9" height="9" stroke="currentColor"/></svg>
              </button>
              <button onClick={() => window.api?.close?.()} style={{ width: 28, height: 22, border: 'none', background: 'transparent', cursor: 'pointer', color: '#999', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M1 1l8 8M9 1l-8 8" stroke="currentColor" strokeWidth="1.2"/></svg>
              </button>
            </div>
          )}
        </div>
